...
function renderQuiz() {
  ...
  el.innerHTML = `<div class="card floating-wrap">${renderFloatingHearts()}<div class="top"><div class="row"><span class="pill">你的底色：${type.code} · ${type.name}</span><span class="pill">第 ${state.questionIndex + 1} / ${QUESTIONS.length} 题</span></div><div class="status">${state.justPicked ? "已选择，正在跳进下一关..." : `${q.sceneTitle} 正在拆你`}</div></div><h1 class="title" style="font-size:32px;">${q.sceneTitle}</h1><p class="sub">${q.sceneSetting}</p><div style="margin-top:18px;"><div class="track"><div class="fill" style="width:${progress}%;"></div></div><div class="prog">当前关卡进度 ${done + 1} / ${scene.questions.length}</div></div></div><div class="card question-shell"><div class="question-kicker">如果这事真发生，你八成会：</div><h2 class="question-title">${q.text}</h2><div class="list">${q.options.map((item, index) => `<div class="option-card" data-option="${index}"><div class="mark">${String.fromCharCode(65 + index)}</div><div class="option-text">${item.text}</div></div>`).join("")}</div></div><div class="actions"><button id="backBtn" class="btn-secondary">返回首页</button></div>`;
